...
if ( ! class_exists( 'CPT_WPLF_Form' ) ) :
    ...
    class CPT_WPLF_Form {
        ...
        /**
         * Before permanently deleting form entry, remove attachments
        ...
        public function metabox_shortcode( $post ) {
            ?>
            <p><input type="text" class="code" value='[libre-form id="<?php echo esc_attr( $post->ID ); ?>"]' readonly>
            </p>
            <?php
        }
        ...
        public function metabox_media_library( $post ) {
            $meta    = get_post_meta( $post->ID );
            $checked = isset( $meta['_wplf_media_library'] ) && ! empty( $meta['_wplf_media_library'][0] ) ? 'checked' : 'false';
